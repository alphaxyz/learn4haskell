...
fight m k
 | mHealt m - kAttack k <= 0 = kGold k + mGold m
 | kHealt k - mAttack m <= 0 = (-1)
 | otherwise = kGold k
...
buildCastle city = case castle city of
    None -> city { castle = OnlyCastle }
    _ -> city

buildHouse h city = city { houses = h : houses city }

countPeople city = sum (map c ( houses city))
 where c h = case h of
              One -> 1
              Two -> 2
              Three -> 3
...
buildWalls city = case castle city of
    OnlyCastle -> if  (countPeople city) >= 10 then city { castle = CastleWithWalls } else city
    _ -> city
...
data DragonLair treasure power = DragonLair
    { dLChest :: Maybe (TreasureChest treasure)
    , dLDragonPower :: power
    }
